...
def buscar_palavra(sheet, coluna, palavra):
    ...
    if numero_coluna < 1:
        print(f"[ERRO] numero_coluna inválido: {numero_coluna}")
        return -1
...
def buscar_palavra_com_linha(sheet, coluna, palavra, linha_inicial, linha_final):
    ...
    # LOG DEBUG: Validar parâmetros
    print(
        f"[DEBUG buscar_palavra_com_linha] coluna={coluna}, numero_coluna={numero_coluna}, palavra={palavra[:50]}..."
    )
    print(
        f"[DEBUG buscar_palavra_com_linha] linha_inicial={linha_inicial}, linha_final={linha_final}, sheet.max_row={sheet.max_row}"
    )
    ...
    if numero_coluna < 1:
        ...
        return -1
    if linha_inicial < 1:
        print(f"[ERRO] linha_inicial inválido: {linha_inicial}")
        return -1
    if linha_final < 1:
        print(f"[ERRO] linha_final inválido: {linha_final}")
        return -1
    if linha_inicial > linha_final:
        print(f"[ERRO] linha_inicial ({linha_inicial}) > linha_final ({linha_final})")
        return -1
...
def buscar_palavra_contem(sheet, coluna, texto, lin_ini, lin_fim):
    ...
    # LOG DEBUG: Validar parâmetros
    print(
        f"[DEBUG buscar_palavra_contem] coluna={coluna}, numero_coluna={numero_coluna}, texto={texto[:30]}..."
    )
    print(
        f"[DEBUG buscar_palavra_contem] lin_ini={lin_ini}, lin_fim={lin_fim}, sheet.max_row={sheet.max_row}"
    )
    ...
    if numero_coluna < 1:
        ...
        return -1
    if lin_ini < 1:
        print(f"[ERRO] lin_ini inválido: {lin_ini}")
        return -1
    if lin_fim < 1:
        print(f"[ERRO] lin_fim inválido: {lin_fim}")
        return -1
    if lin_ini > lin_fim:
        print(f"[ERRO] lin_ini ({lin_ini}) > lin_fim ({lin_fim})")
        return -1
