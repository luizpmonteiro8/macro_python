def valor_bdi_final(sheet, dados, coluna_valor_string, coluna_valor_value):
    ...
    for x in range(2, linha_final + 1):  # Começamos do segundo
        ...
        if valor_Bdi in str(valor):
            ...
            formula_atual = f"=ROUNDDOWN({coluna_valor_value}{x-1}*BDI,2)"
            sheet[f"{coluna_valor_value}{x}"].value = formula_atual

            # Adiciona a fórmula na próxima linha (se não for célula mesclada)
            formula_proxima = f"={coluna_valor_value}{x} + {coluna_valor_value}{x-1}"
            celula_proxima = sheet[f"{coluna_valor_value}{x + 1}"]
            try:
                celula_proxima.value = formula_proxima
            except AttributeError:
                # Célula mesclada - não pode ser modificada
                pass
