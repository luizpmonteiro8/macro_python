...
def salvar_alteracao_json(
    ...
    with open('config/valores_item.json', 'r',
        ...
        if item["nome"] == old_name:
            ...
            for index, (key, value) in enumerate(item.items()):
                if isinstance(value, dict):
                    value['nome'] = var_nome_values[key].get()
                    value['total'] = var_total_values[key].get()
                    value['adicionarFator'] = (var_add_fator_values[key]
                                               .get())
                    value['fatorCoeficiente'] = (var_fator_coef_values
                                                 [key].get())
                    value['buscarAuxiliar'] = (var_buscar_auxiliar_values[key]
                                               .get())
    ...
    if not valor_encontrado:
        newItem = {}
        # adiciona novo item
        newItem['nome'] = var_nome.get()

        for index, (key, value) in enumerate(var_nome_values.items()):
            newItem['item'+str(index+1)] = {
        ...
        dados_json.append(newItem)

    # Escrever os dados de volta no arquivo
    with open('config/valores_item.json', 'w',
              encoding='utf-8') as arquivo_json:
...
def interface_editar_item(self, dropdown_valor_item,
                          updated_item=None):
    ...
    if updated_item:
        ...
        select_item = next((dado for dado in self.todos_item if dado.get(
            'nome') == dropdown_valor_item.get()), None)

    # Variáveis StringVar
    old_name = select_item['nome']
    var_nome = tk.StringVar(
        value=select_item['nome'])
    var_nome_values = {}
    var_total_values = {}
    ...
    # nome
    entry_nome_titulo = custom_input(
        frame, 'Nome', var_nome.get(), row=1)
    entry_nome_titulo.bind("<KeyRelease>", lambda event,
                           v=var_nome: v.set(entry_nome_titulo.get()))
    ...
    for key, value in select_item.items():
        if isinstance(value, dict):
            ...
            entry_nome = custom_input(
                frame, f'Item {items}', var_nome_values[key].get(),
                row=row_counter + 1 + ultima_linha)

            entry_nome.bind("<KeyRelease>", lambda event,
                            v=var_nome_values[key], e=entry_nome:
                                v.set(e.get()))

            # total
            total_entry = custom_input(
                frame, 'Total', var_total_values[key].get(),
                row=row_counter + 2 + ultima_linha
            )

            total_entry.bind(
                "<KeyRelease>", lambda event, v=var_total_values[key],
                e=total_entry: v.set(e.get()))

            # adicionarFator
            label_adicionar_fator = tk.Label(
                frame, text="Adicionar Fator", font=(None, 18))
            label_adicionar_fator.grid(
                row=row_counter + 3 + ultima_linha, column=0, sticky="ew",
            )
            adicionar_fator_dropdown = ttk.Combobox(
                frame, values=[
                    'Sim', 'Não'],
                textvariable=var_adicionar_fator_values[key], font=(None, 18))
            adicionar_fator_dropdown.grid(
                row=row_counter + 3 + ultima_linha, column=1, sticky="ew",
                padx=10)
            ...
            label_fator_coeficiente.grid(
                row=row_counter + 4 + ultima_linha, column=0, sticky="ew",
            )
            ...
            fator_coeficiente_dropdown.grid(
                row=row_counter + 4 + ultima_linha, column=1, sticky="ew",
                padx=10)
            ...
            label_buscar_auxiliar.grid(
                row=row_counter + 5 + ultima_linha, column=0, sticky="ew",
            )
            buscar_auxiliar_dropdown = ttk.Combobox(
                frame, values=[
                    'Sim', 'Não'],
                textvariable=var_buscar_auxiliar_values[key], font=(None, 18))
            buscar_auxiliar_dropdown.grid(
                row=row_counter + 5 + ultima_linha, column=1, sticky="ew",
                padx=10)
            ...
            # separator
            ttk.Separator(frame, orient="horizontal").grid(
                row=row_counter + 6 + ultima_linha, columnspan=2, sticky="ew",
                pady=5
            )
    ...
    canvas_editar.bind("<MouseWheel>", lambda event: mousewheel(
        event, canvas_editar, frame))

    nova_janela.protocol("WM_DELETE_WINDOW",
                         lambda: fechar_janela(canvas_editar, nova_janela))
