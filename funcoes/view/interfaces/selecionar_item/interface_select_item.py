...
def atualizar_interface_select_item(self, menu_dropdown, frame_item,
                                    dropdown_valor_item):
...
def excluir_item(self, dropdown_valor_item, menu_dropdown, frame_item):
    ...
    self.todos_item = [
        item for item in self.todos_item if item["nome"] !=
        dropdown_valor_item.get()
    ]
    # salvar json
    ...
    atualizar_interface_select_item(self, menu_dropdown, frame_item,
                                    dropdown_valor_item)
...
def interface_select_item(self, frame_item, dropdown_valor_item):
    ...
    menu_dropdown = tk.OptionMenu(
        frame_item, dropdown_valor_item,
        *[d["nome"] for d in self.todos_item])
    ...
    for opcao in self.todos_item:
        novo_menu.add_command(
            label=opcao.get("nome"), command=lambda opcao=opcao:
                dropdown_valor_item.set(opcao.get("nome")))
    ...
    btn_editar = tk.Button(
        frame_item, text="Editar",
        command=lambda: interface_editar_item(
            self, dropdown_valor_item),
        font=(None, 18))
    btn_editar.grid(row=0, column=1, pady=10, padx=10)

    btn_adicionar = tk.Button(
        frame_item, text="Adicionar",
        command=lambda: interface_editar_item(
            self, dropdown_valor_item, new_item),
        font=(None, 18))
    btn_adicionar.grid(row=0, column=2, pady=10, padx=10)

    btn_excluir = tk.Button(
        frame_item, text="Excluir",
        font=(None, 18),
        command=lambda: excluir_item(
            self, dropdown_valor_item, menu_dropdown, frame_item,
